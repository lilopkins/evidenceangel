...
impl AppModel {
    fn open_new(&mut self) -> evp::Result<()> {
    ...
    fn open(&mut self, path: PathBuf) -> evp::Result<()> {
    ...
    fn update_nav_menu(&mut self) -> evp::Result<()> {
            ...
            AppInput::__OpenFile(path) => {
                if let Err(e) = self.open(path.clone()) {
                    ...
                    let error_dlg = ErrorDialogModel::builder()
                        ...
                        .forward(sender.input_sender(), |msg| match msg {});
                    if matches!(e, evp::Error::LockNotObtained) {
